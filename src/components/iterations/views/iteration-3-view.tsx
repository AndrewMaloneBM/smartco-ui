...
export function Iteration3View({ scenario }: { scenario?: string | null } = {}) {
  ...
  // STEP3_SCENARIOS starts empty (Step 2's scenarios aren't repeated here) —
  // cases are added below one at a time as each Step 3 PRD feature lands. A
  // null scenario (no cases yet, or "Default view") falls through to the
  // clean baseline: a freshly reseeded, reset view.
  ...
  useEffect(() => {
    ...
    switch (scenario) {
      case "grade-create":
        setCreateInitial({ campaignName: "Excellent-grade smartphones push", grades: ["EXCELLENT"] });
        setCreateOpen(true);
        break;
      ...
      <div className="flex flex-wrap items-start gap-3">
        ...
        <div className="w-44">
          <RevSelect
            ...
            onChange={(labels) => {
              // PRD: Offer type is single-select. RevSelect is a shared multiselect
              // primitive, so we keep only the most recently toggled label — picking a
              // second option replaces the first, and unchecking clears to [].
              const codes = labels
                .map((l) => OFFER_TYPES.find((o) => o.label === l)?.code)
                .filter((c): c is OfferTypeCode => c !== undefined);
              set("offerType", codes.slice(-1));
            }}
          />
        </div>
